...
├── tools/
│   ├── test_store.js                    ٥٨ اختبارًا — node tools/test_store.js
...
```gradle
versionCode 12       // رقم صحيح يزيد دائمًا — أندرويد يمنع التحديث بدونه
versionName "2.1"    // ما يراه المستخدم
```
...
`pv`, `lib:<kind>`, `cat:<kind>`, `grp:<kind>`, `grp:<kind>:<groupId>`.
...
٥٨ اختبارًا، بلا أي حزم خارجية. الفكرة: تشغيل `app.js` **الحقيقي** داخل
...
والإسناد وإعادة التسمية والحذف والترتيب والزرع مرّة واحدة).
