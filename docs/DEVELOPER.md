...
│       │   └── library.js      (592 س)  ٢٣٢ تحليلًا + ٢٧٠ علاجًا مصنّفة
...
├── tools/
│   ├── test_store.js                    ٣٤ اختبارًا — node tools/test_store.js
...
```gradle
versionCode 7        // رقم صحيح يزيد دائمًا — أندرويد يمنع التحديث بدونه
versionName "1.6"    // ما يراه المستخدم
```
...
| 574–695 | 💊 العلاجات | قائمة + نموذج + حفظ + حذف |
| 697–790 | 🧪 التحاليل | نفس النمط |
| 792–893 | 🌿 الوصفات | نفس النمط |
...
```js
DB = {
  meds: [], labs: [], recipes: [],        // العناصر
  cart: { meds: [], labs: [], recipes: [] },
  groups: [],                             // {id, kind, name, items[]}
  out: { meds: [], labs: [], recipes: [] },// الحقول المرسلة
  pin_hash: null
}
```
...
أسماء الصفحات: `home`, `meds`, `labs`, `recipes`, `settings`,
`lib:meds`, `lib:labs`, `grp:<kind>`, `grp:<kind>:<groupId>`.
...
٣٤ اختبارًا، بلا أي حزم خارجية. الفكرة: تشغيل `app.js` **الحقيقي** داخل
